...
def get_tensor_dataset_tiered(dataset, max_sentences, add_segment_ids=False):
  ...
  for ex in dataset:
    ...
    for story in ex['stories']:
      entity_list = [story['entities'][i]['entity'] for i in range(len(story['entities']))]
      entity_list_i.append(entity_list)
      graph_list = story['dep_graph']
      graph_list_i.append(graph_list)
      graph_label = story['label_list']
      graph_label_i.append(graph_label)

    label_entity.append(entity_list_i)
    dep_graph_list.append(graph_list_i)
    graph_labels.append(graph_label_i)
  ...
  print('-----------------')
  # print(num_entities)
  print(len(dep_graph_list))
  print(len(dep_graph_list[0]))
  print(len(graph_labels))
  print(len(graph_labels[0]))
  ...
  return tensor_dataset, label_entity, dep_graph_list, graph_labels
    ...
    for i, ex_2s in enumerate(dataset[p]):
      for s_idx, ex_1s in enumerate(ex_2s['stories']):
        for ent_idx, ex in enumerate(ex_1s['entities']):
          ...
          print(dataset[p][i]['stories'][s_idx]['entities'][ent_idx]['input_ids'].shape)
...
def extract_graph(sentence,reference):
  ...
  jar_path = '/scratch/drjieliu_root/drjieliu/zheyuz/Verifiable-Coherent-NLU/stanford-corenlp-4.2.2/stanford-corenlp-4.2.2.jar'
  models_jar_path = '/scratch/drjieliu_root/drjieliu/zheyuz/Verifiable-Coherent-NLU/stanford-corenlp-4.2.2-models-english.jar'
  ...
  #     del labels[key]
  #   else:
  #     node_attr.append(reference[value.lower()])
  node_attr=[]
  for key in list(labels.keys()):
    try:
      node_attr.append(reference[labels[key].lower()])
    ...
    for ex in tqdm(tiered_dataset[p]):
      for story in ex['stories']:
        ...
        for sent in story['sentences']:
          extracted_g, extracted_label = extract_graph(sent,reference=cn_nb)
...
# def get_tensor_depend_graph(tiered_dataset):
