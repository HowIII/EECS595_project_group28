...
def train_epoch_tiered(model, optimizer, train_dataloader, device, seg_mode=False, return_losses=False, build_learning_curves=False, val_dataloader=None, train_lc_data=None, val_lc_data=None,dep_graphs=None, label_entities=None,graph_labels=None, cn_nb=None):
  t0 = time.time()
  ...
  for step, batch in enumerate(train_dataloader):
    ...
    print('Input entities: {}'.format(input_entities))
    ...
    dep_graphs_i = dep_graphs['train'][step]
    labels_entities_i = label_entities['train'][step]
    graph_labels_i = graph_labels['train'][step]
    ...
    out = model(input_ids, 
                ...
                labels=labels,
                training=True,
                dep_graphs_i=dep_graphs_i,
                labels_entities_i=labels_entities_i,
                graph_labels_i=graph_labels_i,
                cn_nb=cn_nb)
    ...
    if build_learning_curves:
      ...
      if (len(train_dataloader) - step - 1) % chunk_size == 0:
        validation_results = evaluate_tiered(model, val_dataloader, device, [(accuracy_score, 'accuracy'), (f1_score, 'f1')], seg_mode=False, return_explanations=True, return_losses=True, verbose=False,dep_graphs=dep_graphs['dev'],label_entities=label_entities['dev'],graph_labels=graph_labels['dev'],cn_nb=cn_nb)
